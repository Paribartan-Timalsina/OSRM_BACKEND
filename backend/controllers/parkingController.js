...
exports.confirmparking = tryCatchAsync(async (req, res, next) => {
  const { userId, locationId, vehicleType, startTime, parkingtime,reservedSpaceNumber } = req.body;
...
console.log(parkingtime)
  ...
  const expirationTime = new Date(startTimehere.getTime() + parkingtime * 60000);
...
exports.mybookingDetails=tryCatchAsync(async (req,res,next)=>{
  ...
  UserReservation.find({userId:userId}).populate('userId').then(p=>console.log(p))
})
